...
function mapStateToProps(state) {
    ...
    return {
        ...
        allCompanies: state.adminReducer.allCompanies,
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        remove: (data) => dispatch(CompanyAction.removeData(data)),
                ...
                {/* <InfoDialog name={this.state.name} department={this.state.department} open={this.state.openModal} handleOpen={this.handleModal} /> */}
                {
                    this.props.isProgressing ? (
                        ...
                        <div>
                            ...
                            <List>
                                {
                                    (this.props.allCompanies.length > 0)?
                                    this.props.allCompanies.map((eachUser, i) => {
                                        return (
                                            <ListItem style={{backgroundColor:'#dedede'}} key={i} >
                                                <h1>Company Name: {" " + eachUser.companyName}</h1>
                                                <h2>Vacancy: {" " + eachUser.vacancy}</h2>
                                                <h2>Salary: {" " + eachUser.salary}</h2>
                                                <div>
